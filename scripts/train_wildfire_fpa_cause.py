...
def _make_loaders(
    ...
    if imbalance == "weighted_sampler":
        ...
        # optional: weights for loss too (not required)
...
def _parse_unexpected_kwargs(msg: str) -> List[str]:
    # pattern: "Unexpected kwargs for wildfire_fpa_mlp: ['num_layers']"
    ...
    if m:
        try:
            ...
            return []
    # pattern: "got an unexpected keyword argument 'task'"
...
def _build_with_stripping(builder, kwargs: Dict[str, Any]) -> Tuple[nn.Module, Dict[str, Any]]:
    """
    Keep retrying by stripping kwargs that the builder explicitly rejects.
    This makes us robust to API changes like num_layers vs depth.
    """
    cur = dict(kwargs)
    for _ in range(20):
        try:
...
def build_wildfire_fpa_mlp_strict(
    ...
    # Start with the kwargs that worked for you: in_dim/out_dim/hidden_dim/dropout/depth (+ task).
    # Then strip anything the builder rejects (task or depth naming variations).
    base = {
        "task": task,
        "in_dim": in_dim,
        "out_dim": out_dim,
        ...
        "dropout": dropout,
        "depth": depth,
        # Also try a couple synonyms in case a future builder changes:
        # If these are rejected, they get stripped automatically.
        "num_layers": depth,
        "layers": depth,
    }
    ...
    # record only keys that survived
    used_clean = dict(used)
    return model, used_clean
...
class FocalLoss(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        p = torch.exp(logp)
        tgt = target.long()
        logp_t = logp.gather(1, tgt.unsqueeze(1)).squeeze(1)  # [B]
        p_t = p.gather(1, tgt.unsqueeze(1)).squeeze(1)        # [B]
...
class LitClassifier(pl.LightningModule):
    def __init__(
        ...
        if loss_name == "focal":
            ...
            # label_smoothing supported in modern torch; if not, ignore
            try:
...
def main() -> None:
    ...
    ap.add_argument("--cause_mode", choices=["paper5", "full12", "keep_all"], default="paper5")
    ...
    ap.add_argument("--out_root", default="outputs/train_wildfire_fpa_cause")
    ...
    try:
        ...
        _plot_cm(cm, classes, os.path.join(run_dir, "confusion_matrix.png"), "Cause confusion matrix")

        torch.save(lit.model.state_dict(), os.path.join(run_dir, "model_final.pt"))

        y_counts = {classes[i]: int((y_train == i).sum()) for i in range(num_classes)}
        ...
        summary = [
            "# Paper reproduction summary — FPA-FOD Cause",
            "",
            "## Dataset",
            f"- train: {x_train.shape}  val: {x_val.shape}  test: {x_test.shape}",
            f"- classes: {classes}",
            f"- train class counts: {y_counts}",
            "",
            "## Model kwargs actually used (strict builder)",
            f"- {used_kwargs}",
            "",
            "## Your test metrics",
